...
int    n_mu      =    20;      // number of theta bins
int    n_phi     =    1;      // number of phi bins

// grid parameters
double r_inner   =  0.1;      // inner boundary radius, in kpc
double r_outer   =  1.0;      // outer boundary radius, in kpc
double r_emit    =  0.1;      // boundary to emit from
double n_0       =  1e-3;     // number density at inner boundary
double n_law     =    2;      // -1*power law exponent of density law
double v_max     =  3.0e7;    // velocity at outer boundary (cm/s)
double v_law     =    1;      // power law of velocity profile
double bipolar   =    0;      // degree of bipolarity

double dust_cs     = 3.33e-22;    // dust cross-section
double dust_dens   =  1;          // density of NH/dust, 1 gives tau=1
...
double Get_Density(double *x, double r)
{
  ...
  double mu = x[2]/r;
  return n_0*pow(r_inner/r,n_law)*pow(mu*mu,bipolar);
}
