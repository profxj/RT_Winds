...
void EMIT_LINE::Compute_Cumul()
{
  ...
  double norm = EW_x * ( gsl_sf_erf(u2) - gsl_sf_erf(u1))/2. + (wave[n_x-1]-wave[0]);
  ...
  for (i=0; i<n_x; i++) {
    ...
    intg[i] = EW_x * ( gsl_sf_erf(uval) - gsl_sf_erf(u1))/2. + (wave[i]-wave[0]);
    intg[i] = intg[i]/norm;
