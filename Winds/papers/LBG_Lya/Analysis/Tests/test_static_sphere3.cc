//  Continuum source; constant density sphere  
...
double n_0       =  6.4815577e-3; // Gives 2x10^20 cm^-2
...
double v_doppler    =   18.24*1e5; // cm/s (T=20,000K)
...
void Run_Monte_Carlo(char *outfile)
{
  ...
  for (i=0;i<n_photons;i++)
  {
    ...
    // initial wavelength (continuum source)
	...
	if (atau > 60)     xcr = 0.04*exp(1.4*pow(log(atau),0.6));
