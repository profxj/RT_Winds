;; Makes the Grid for a biconical model
;;  v ~ r
;;  n ~ r^-2
;;  Front half only
pro grid_biconical,theta_min

  if not keyword_set(THETA_MIN) then return
...
;  dv_wind = 200.           ; Velocity width of wind (km/s)
  ...
  theta = reform( atan( abs(cell_center[*,*,*,2]), $
                        sqrt(cell_center[*,*,*,0]^2 + cell_center[*,*,*,1]^2) ),$
  ...
  ;; Output
  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  outfil = 'wind_grid_biconic_'+strtrim(round(theta_min),2)+'.asc'
  close, /all
  openw, 11, outfil
  printf, 11, ngrid, dl
