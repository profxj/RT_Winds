...
;  dv_wind = 200.           ; Velocity width of wind (km/s)
  ...
  ;; ISM
  msk_corners[*] = 0
  gd_corn = where(r_corners LE rw_inner and r_corners GE r_ISM)
  msk_corners[gd_corn] = 1
  frac_cell = total(msk_corners,4)
  rho_grid = rho_grid + n_ISM * float(frac_cell) / 8.
  delvarx, msk_corners
  ...
  gd_cell = where(r_cell GE rw_inner, $
  ...
  ISM_cell = where(r_cell LT rw_inner and r_cell GT rg_outer)
  dopp[ISM_cell] = b_ISM
