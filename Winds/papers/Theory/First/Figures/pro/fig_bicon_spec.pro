pro fig_bicon_spec, RREAL=rreal

  if not keyword_set( PSFILE ) then psfile = 'fig_bicon_spec.ps'
  ...
  if not keyword_set(FeII_grid) then $
     FeII_grid =  '../Analysis/Asymmetric/biconic_feII_grids.fits'
  if not keyword_set(MgII_grid) then $
     MgII_grid = '../Analysis/Asymmetric/biconic_mgII_grids.fits'
  if not keyword_set( GRID_FILE ) then $
    grid_file = '../Analysis/Outputs/fiducial_grid.fits'

  if not keyword_set(ALL_ANG) then all_ang = [10, 45, 60]
  if not keyword_set(ANGLES) then angles = [10, 45, 60]
  if not keyword_set(FE_ANGLES) then fe_angles = [10, 45, 60]
...
;     for kk=0L,-1 do begin
        ...
        if ss EQ 0 then $
           xyouts, xrng[0]+xlbl2*(xrng[1]-xrng[0]), yrng[1]-(kk+2)*ystp*(yrng[1]-yrng[0]), $
                   '!9q!X='+strtrim(fe_angles[kk],2), color=clrs[kk+1], charsiz=lsz
     endfor
