pro fig_norm_spec, RREAL=rreal

  if not keyword_set( PSFILE ) then psfile = 'fig_norm_spec.ps'
  ...
  close, /all
  openr, 1,'Input/fig_norm_spec.inp'

  ;;; BEGIN PLOTS
  x_psopen, psfile, /portrait
  !p.multi = [0,1,2]
  clr = getcolor(/load)
  clrs = [clr.blue, clr.black, clr.red, clr.darkgreen] ;x_setclrs()
  ...
  for ss=0,1 do begin
     ...
     for kk=0L,nFe-1 do begin
        ...
        ;; Parse
        i1pos = strpos(Fe_fil, 'norm_')
        i2pos = strpos(Fe_fil, '.dat')
        if i1pos LT 0 then norm = 1. else $
           norm = float(strmid(Fe_fil, i1pos+5, i2pos-i1pos+2))/10.
;        print, 'Reading', fe_fil
  ...
  xyouts, xrng[0]+xlbl*(xrng[1]-xrng[0]), yrng[0]+(yrng[1]-yrng[0])*ylbl, $
          'FeII', color=clr.black, charsiz=lsz2
;     oplot, xrng, [1., 1.], color=clr.red, linestyle=1, thick=1
  ...
  for kk=0L,nMg-1 do begin
     readf, 1, Mg_fil
     ;; Parse
     i1pos = strpos(Mg_fil, 'norm_')
     i2pos = strpos(Mg_fil, '.dat')
     if i1pos LT 0 then norm = 1. else $
        norm = float(strmid(Mg_fil, i1pos+5, i2pos-i1pos+2))/10.
  ...
  xyouts, xrng[0]+xlbl*(xrng[1]-xrng[0]), yrng[1]*ylbl, $
          'MgII', color=clr.black, charsiz=lsz2
  oplot, xrng, [1., 1.], color=clr.red, linestyle=1, thick=1
