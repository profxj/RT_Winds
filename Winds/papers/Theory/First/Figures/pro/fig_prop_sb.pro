pro fig_prop_sb, SHOW_INT=show_int

  if not keyword_set(PSFILE) then psfile = 'fig_prop_sb.ps'
  if not keyword_set(GRID_FILE) then $
     grid_file =  '../Analysis/Outputs/fiducial_grid.fits'
  if not keyword_set(LBG_FILE) then $
     lbg_file =  '../Analysis/LBG/Covering/3D/Models/lbg_s10_grid_mgii.fits'
...
;  xmrg = [9,7]
  ...
  plot, [0], [0], color=clr.black, background=clr.white, charsize=csz,$
        ...
        ytitle='Normalized Flux Profile', yrange=yrng, thick=4, $
  ...
  for ss=0,1 do begin
     ...
     ;; Read in Data
     raw_data = xmrdfits(wind_file, idx[0], /silent)
     raw_wave = xmrdfits(wind_file, idx[1], /silent)
...
;     if ss EQ 1 then stop
     ...
     if ss EQ 0 then neval = sz_raw[0]/2 else neval = sz_raw[0]
     avg_r = fltarr(neval)
     avg_mu = fltarr(neval)
     ...
     ;; Plot
;     oplot,  avg_r, avg_mu_kpc, color=clr.black, psym=10, linesty=lsty
     oplot,  avg_r, avg_f/mx_af, color=clr.black, linesty=lsty
;     if ss EQ 1 then stop

     ;; Label
     oplot, [0.7,0.74]*xrng[1], replicate(yrng[1]/(2^(ss+2)), 2), color=clr.black, $
            linesty=lsty
     xyouts, 0.75*xrng[1], yrng[1]/(2^(ss+2))/1.1, l_model, $
