...
\title{Diagnosing Simple Wind Models of Cool Gas Outflows through
  Metal-Line Absorption and Emission}
...
observations.  We focus on the \mgiid\ doublet and \ion{Fe}{2} UV1
multiplet at $\lambda \approx 2600$\AA, but the results are applicable
to most other transitions used to study outflows (e.g.\ \ion{Na}{1},
\ion{H}{1} \lya, \ion{Si}{2}).   We implement monte carlo radiative
transfer techniques to properly track the propogation of scattered and
flourescent photons to [produce] 1D spectra and 2D spectral images.
[Should we make the codes publically available?  Upon request?]
[Mention fiducial model here?]
By design, the \ion{Mg}{2} doublet exhibits strong, blue-shifted
absorption but also comparably strong line-emission at roughly
systemic velocity.  This line-emission `fills-in' the absorption at
velocieties near systemic, significantly reducing the inferred
opacity. Analysis of cool gas outflows that entirely ignores this
...
The \ion{Fe}{2} resonance lines show similar behaviour, however 
these lines are connected to fine-structure levels that are optically
thin to the medium.  As a result, the resonant line-emission is
reduced (at the `expense' of significant fine-structure emission) and
the absorption lines more faithfully reproduce the intrinsic opacity
...
profiles and a radiative-driven wind model.  The resultant \ion{Mg}{2}
and \ion{Fe}{2} profiles show a diverse complexity of absorption and
emission characteristics but overall have qualitatively similar
features.  We also study a scenario purported to describe the outflows
of $z \sim 3$ Lyman break galaxies and argue this model fails to
reproduce the observations when (previously ignored0 scattered photons
are properly considered.   Strong line-emission from metal-line
transitions is a generic prediction of wind models, even in the
presence of dust and anisotropic configurations.  We calculate the
surface brightness profiles of this emission which motivates
spatially-extended spectroscopic observations with current and
next-generation integral-field-unit instruments.
...
not be sufficient to preclude the detection. 
...
flux falls by only a factor of 10 from $r = 1$ to 15\,kpc.
...
phenomena.  The next generaly of large-format optical and infrared
...
profile of the wind.  Although these effects are modest (tens of
\kms), they can be [recorded] by modern resolution spectroscopy.

We also find that the emission-line widths reveal
...
resonantly trapped.  [One last example?]
...
models.  Our analysis focused on the \mgiid\ and \ion{Fe}{2} UV1
multiple of rest-frame UV transitions, but the results translate to
most other lines thus far used to probe these phenomna.  Our primary
...
\item Isotropic, dust-free wind models conserve photon flux.
  ...
  emission-lines with similar equivalent width.  
...
\item The line-emission occurs preferentially at the systemic velocity
  ...
  \ion{Na}{1}), one predicts this emission to significantly fill-in
  the absorption near systemic.
...
non-resonant lines (e.g.\ \ion{Fe}{2}, \ion{Si}{2}) offer the best
characterization of the opacity of the wind and also any component
with velocity near systemic.  The emission will be domianted by the
optically-thin, fine-structure transitions.

\item  Dust extinction has only modest effects on the profiles
  in models where one to only a few scatterings are predicted.  Models
  with high opacity at small radii and at systemic velocity (e.g.\ an
  optically thick, ISM component)  can effecitvely extinguish
...
\item We examined two scenarios designed to mimic the wind model
  ...
  greatly modify the predicted line-profiles.  As such, 
