...
up to $\sim 10$ kpc
%tens of kpc 
from the galaxy \citep{M87,lhw99,wsg08,vsr+03} carrying significant speed
to escape from the gravitational potential well of the galaxy's dark
matter halo \citep{wind_escape,sh09}.
...
robust measurements of the speed of the gas (is this really true?), 
...
\citep{debate,mqt05,sdr08,sh09,ssr02,od06,kkd+09}.
...
into our sightline, and are analogous to the emitted photons in a P
Cygni profile.
...
fluorescence for the first time.  
...
[behavior] is a generic result, even for wind models that include
...
includes \ion{Na}{1} flows \cite{rvs05a,martin05,smn+09}, 
...
preferentially be lost (???) relative to a non-resonant photon.  In
...
These negative effects for \ion{Mg}{2} absorption are reduced
...
\ion{Fe}{2}.  The \ion{Mg}{2} lines (especially \mgiia) has the
...
[Centroid of \ion{Mg}{2} $-->$ dust, isotropy]
...
with slit width and plateaus at $\approx \mrtt$.  [Add?]
}
\label{fig:obs_slit}
\end{figure}
...
\gtrsim -400 \mkms$.  Therefore, the effects are greatest for wind
...
profiles have greatest depth.  Results for all the models
...
systemic velocity have a \mgiib\ absorption profile shifted blueward
...
shorter wavelength (a greater velocity offset from systemic).  
