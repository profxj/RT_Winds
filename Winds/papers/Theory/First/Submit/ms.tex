...
millions of energy levels recorded \citep{kurucz_cd1}, and even this is an
incomplete list.  
...
may by much smaller than the length scale of the
...
this optical depth is independent of the direction of propagation and given by
\begin{equation}
\tau^S(r) = K^\ell   \lambda_0^\ell | dv/dr |^{-1} \cmma
\label{eqn:Sobolev}
\end{equation}
where 
\begin{equation}
K^\ell = \frac{\pi e^2}{m_e c} f^\ell n^\ell(r)
\end{equation}
is the integrated line opacity, $\lambda_0^\ell$ the rest center wavelength,
$f^\ell$ the oscillator strength and
$n^\ell$ the density in the lower level of the transition.   We have
neglected corrections for stimulated emission.   This optical
depth applies to a photon with wavelength $\lambda^S
\equiv \lambda^\ell (1- v(r)~\hat{r}  \cdot \hat{d} /c)$ where $\hat{d}$ is the 
direction of propagation.  The probability that such a photon
is scattered/absorbed at the point of resonance is simply $1 - \exp[-\tau^S(r)]$.
...
% $\tau_{2796}$ profile
%  I don't understand this sentence so I commented it out -- ? -- Dan 
For the spatial integrated (i.e., 1D) spectra, the actual dimensions and density of the wind are
...
diagnostic on the degree to which the resonance line photons are trapped.
...
in this model the central source is of relatively small  and the radiation
...
\begin{equation}
n_{\rm LBG}(r) =  \tau_{\rm LBG}(r)  \frac{ |d\mvr/dr |}{f^\ell  \lambda_0^\ell}
