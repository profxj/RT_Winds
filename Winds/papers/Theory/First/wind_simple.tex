...
`removed' the $v \approx 0\kms$ absorption component.  Although the
...
At high redshift, \ion{Si}{2}* line emission has been reported from
the Lyman break galaxies which was suggested to be
related to \ion{H}{2} regions within the galaxies \citep{shapley03}.
Lastly, \ion{Na}{1} line emission has also been detected in the
...
includes the \ion{Na}{1} flows reported by \cite{rupke} and
\cite{martin}, the \ion{Mg}{2} and \ion{Fe}{2} transitions studied in
ULIRGS by \cite{mb10}, and the extreme \ion{Mg}{2} outflows identified
...
the fiducial model]  Although the \ion{Mg}{2} photons are resonantly
...
modest effect.  This reflects the [modest] opacity of the \ion{Mg}{2}
...
Another reason \ion{Mg}{2} is less susceptible than \lya\ is that the
...
freely.  A wind model that effectively trapped \ion{Mg}{2}
...
majority of cases with $v_\tau < -300\mkm$, one observes $\tau_{pk} <
...
All of these (negative) effects for \ion{Mg}{2} absorption are reduced
...
predicts a velocity offset between the peak depth of the \ion{Fe}{2}
centroid and that for \ion{Mg}{2} (Figure~\ref{fig:obs_vtau})
especially when the peak occurs at $v > -400 \mkms$.  One is simply
not sensitive to opacity at $v \sim 0 \mkms$ with the \ion{Mg}{2}
...
shows one realization of the \mgiid\ doublet for the fiducial model
convolved with the line-spread-function of the Keck/LRIS spectrometer
(FWHM=250\kms) and an assumed S/N=7 per XX\AA\ pixel.  Both the absorption
...
averaged 200 identical \ion{Mg}{2} profiles from the fiducial model
degraded to a S/N=1\,pix$^{-1}$ and assuming a random redshift error
$\sigma(z) = 40\mkms$.  [Check S10]
